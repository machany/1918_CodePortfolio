...
namespace AgamaLibrary.Methods
{
    public static class LidraryMethod
    {
        ...
        public static bool MergeTo<T>(this ICollection<T> mergeTarget, ICollection<T> mergeValue)
        {
            bool success = false;

            foreach (T item in mergeValue)
            {
                if (item != null && !mergeTarget.Contains(item))
                {
                    success = true;
                    mergeTarget.Add(item);
                }
            }
